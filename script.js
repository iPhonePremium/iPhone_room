...
/* =========================================================
   FINAL PATCH v2 // REAL FULLSCREEN INSANITY ENGINE
   Fixes the previous invisible-overlay failure by making the
   overlay an actual HTML element, moving it to the very end of
   <body>, and using a dedicated viewport renderer.
   ========================================================= */
(() => {
  'use strict';

  const OV = document.getElementById('insanityOverlay');
  const BTN = document.getElementById('insanityBtn');
  const RESET = document.getElementById('resetBtn');
  const TIMER = document.getElementById('insanityTimer');
  const FILL = document.getElementById('insanityTimerFill');
  const STATUS = document.getElementById('insanityStatus');
  const COORD = document.getElementById('insanityCoordinates');
  const TEXT = document.getElementById('insanityTextField');
  const CANVAS = document.getElementById('insanityNoiseCanvas');

  if (!OV || !BTN || !TIMER || !FILL || !TEXT || !CANVAS) return;

  // Always make this the final child. It is therefore not trapped inside
  // a section, a transformed parent, a grid item, or a stacking context.
  ...
  const ZALGO_TOP = [
    '̍','̎','̄','̅','̿','̑','̆','̐','͒','͗','͑','̇','̈','̊','͂','̓','̈́','͊','͋','͌','̃','̂','̌','͐','̀','́','̋','̏','̒','̓','̔','̽','̉'
  ];
  const ZALGO_MID = ['̕','̛','̀','́','͘','̡','̢','̧','̨','̴','̵','̶','̷','̸','͜','͝','͞','͟','͠','͢'];
  const ZALGO_LOW = ['̖','̗','̘','̙','̜','̝','̞','̟','̠','̤','̥','̦','̩','̪','̫','̬','̭','̮','̯','̰','̱','̲','̳','̹','̺','̻','̼','͇','͈','͉','͍','͎','͓','͔','͕','͖','͙','͚','̣','̥'];
  const GLYPHS = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789#@%&$!?/\\|<>[]{}░▒▓█■□△▽○●※†‡∞∅';
  const PHRASES = [
    '見てる','まだいる','うしろ','そこ','近い','近いよ','聞こえる？','逃げないで',
    '閉じないで','戻れない','見つけた','ここにいる','画面の外','反射が違う',
    '時計が遅れた','音が消えた','誰かいる','目をそらすな','まだ終わらない',
    '名前を呼んだ','今そこにいる','こっちを見て','気づいた？','開けて',
    '閉めて','記録されました','観測中','接続中','切断失敗','■■■■','▒▒▒▒▒',
    'NO ESCAPE','LOOK BACK','SIGNAL LOST','USER FOUND','WATCHING','DO NOT BLINK'
  ];
  ...
  function corrupt(text, amount = 0.42) {
    let out = '';
    for (const ch of text) {
      out += ch;
      if (Math.random() < amount) {
        const count = 1 + ((Math.random() * 5) | 0);
        for (let i = 0; i < count; i++) out += pick(ZALGO_TOP);
        if (Math.random() < .55) out += pick(ZALGO_MID);
        if (Math.random() < .65) out += pick(ZALGO_LOW);
      }
    }
    return out;
  }

  function makeNoiseString() {
    const mode = Math.random();
    if (mode < .34) return corrupt(pick(PHRASES), .25 + Math.random() * .55);
    if (mode < .64) {
      let s = '';
      const len = 4 + ((Math.random() * 18) | 0);
      for (let i = 0; i < len; i++) s += pick(GLYPHS);
      return s;
    }
    return corrupt(pick(PHRASES) + ' // ' + pick(PHRASES), .18 + Math.random() * .45);
  }
  ...
  function paintNoise() {
    ...
    // Low-resolution random blocks: much cheaper than painting every pixel.
    const density = Math.min(900, Math.floor(w * h / 850));
    for (let i = 0; i < density; i++) {
      const x = Math.random() * w;
      const y = Math.random() * h;
      const rw = 1 + Math.random() * (Math.random() < .08 ? 40 : 5);
      const rh = 1 + Math.random() * (Math.random() < .08 ? 8 : 2);
      const r = Math.random();
      if (r < .42) CTX.fillStyle = `rgba(255,0,45,${Math.random() * .14})`;
      else if (r < .62) CTX.fillStyle = `rgba(0,230,255,${Math.random() * .10})`;
      else CTX.fillStyle = `rgba(255,255,255,${Math.random() * .10})`;
      CTX.fillRect(x, y, rw, rh);
    }

    // Horizontal corruption bars.
    const bars = 2 + ((Math.random() * 9) | 0);
    for (let i = 0; i < bars; i++) {
      const y = Math.random() * h;
      const bh = 1 + Math.random() * 14;
      CTX.fillStyle = Math.random() < .7
        ? `rgba(255,0,50,${Math.random() * .12})`
        : `rgba(0,240,255,${Math.random() * .09})`;
  ...
  function createParticle() {
    ...
    el.className = 'madNoiseText';
    const text = makeNoiseString();
    el.textContent = text;
    el.dataset.noise = text;

    const x = random(-3, 92);
    const y = random(-2, 98);
    const life = random(.45, 2.5);
    el.style.left = x + 'vw';
    el.style.top = y + 'vh';
    el.style.setProperty('--madlife', life + 's');
    el.style.setProperty('--mx0', random(-5, 5) + 'vw');
    el.style.setProperty('--my0', random(-4, 4) + 'vh');
    el.style.setProperty('--mx1', random(-10, 10) + 'vw');
    el.style.setProperty('--my1', random(-10, 10) + 'vh');
    el.style.setProperty('--mx2', random(-45, 45) + 'vw');
    el.style.setProperty('--my2', random(-35, 35) + 'vh');
    el.style.setProperty('--mr0', random(-20, 20) + 'deg');
    el.style.setProperty('--mr1', random(-5, 5) + 'deg');
    el.style.setProperty('--mr2', random(-80, 80) + 'deg');
    el.style.setProperty('--ms', random(1.2, 3.5));

    TEXT.appendChild(el);
    window.setTimeout(() => el.remove(), (life + .25) * 1000);
  }

  function burst(count = 12) {
    for (let i = 0; i < count; i++) {
      window.setTimeout(createParticle, i * random(3, 22));
    }
  }
  ...
  function flash() {
    ...
    f.className = 'insaneFlash';
    f.style.background = Math.random() < .5 ? '#fff' : '#ff003c';
    f.style.opacity = String(random(.08, .34));
    OV.appendChild(f);
    window.setTimeout(() => f.remove(), 120);
  }
  ...
  function unlockViewport() {
    document.documentElement.classList.remove('insane-lock');
    if (!saved) return;
  ...
  const blocked = ['wheel','touchmove','touchstart','touchend','pointerdown','pointerup','contextmenu','dragstart','selectstart'];
  blocked.forEach(type => window.addEventListener(type, preventInput, {capture:true, passive:false}));
  ...
  window.addEventListener('scroll', () => {
    if (running && saved) window.scrollTo(0, saved.scrollY);
  }, {passive:true});
  ...
  function update(now) {
    ...
    if (left <= 4000) {
      OV.dataset.phase = 'critical';
      STATUS.textContent = pick([
        'INPUT REJECTED // SIGNAL OVERLOAD',
        '操作不能 // 画面がこちらを見ています',
        'FINAL COUNTDOWN // DO NOT BLINK',
        '▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒▒',
        'LOCK REMAINS // もう少し'
      ]);
    } else {
      OV.dataset.phase = 'normal';
      STATUS.textContent = pick([
        '入力を受け付けていません。',
        'VIEWPORT LOCKED // SCROLL DISABLED',
        '操作経路を一時遮断しています。',
        '画面外へ逃げても表示は解除されません。'
      ]);
    }

    COORD.textContent = `X:${String((Math.random()*999)|0).padStart(3,'0')} Y:${String((Math.random()*999)|0).padStart(3,'0')} // VIEWPORT:${window.innerWidth}×${window.innerHeight} // SIGNAL:${pick(['████','███▒','██▒▒','█▒▒▒'])}`;

    paintNoise();
    if (Math.random() < .55) createParticle();
    if (Math.random() < .055) burst(4 + ((Math.random()*8)|0));
    if (Math.random() < .018) flash();
  ...
  function launch() {
    ...
    // Force the actual overlay to be the final body child AGAIN at activation time.
    document.body.appendChild(OV);
    ...
    document.title = '■■ 発狂 // VIEWPORT LOCK ■■';
    if (typeof sanity !== 'undefined') {
      sanity = 0;
      document.getElementById('sanFill').style.width = '0%';
      document.getElementById('sanVal').textContent = '0%';
    }

    TIMER.textContent = '12.0';
    FILL.style.transform = 'scaleX(1)';
    STATUS.textContent = 'FULLSCREEN LOCK INITIALIZING...';
    TEXT.innerHTML = '';
    ...
    particleTimer = window.setInterval(() => {
      if (running) burst(2 + ((Math.random()*7)|0));
    }, 130);
    flashTimer = window.setInterval(() => {
      if (running && Math.random() < .34) flash();
    }, 360);

    if (typeof addHorrorEvent === 'function') addHorrorEvent('INSANITY V2 // TRUE VIEWPORT LOCK ACTIVE', true);
  }

  function release(reason='timeout') {
    if (!running) return;
    running = false;
    cancelAnimationFrame(raf);
    ...
    window.setTimeout(() => {
      ...
      OV.dataset.phase = 'normal';
      document.title = originalTitle;
      if (typeof updateSan === 'function') updateSan(35);
      if (typeof addHorrorEvent === 'function') addHorrorEvent('INSANITY V2 // AUTO RELEASE COMPLETE');
    }, 900);
  }

  // Replace all earlier button handlers with the final implementation.
  BTN.onclick = launch;
  BTN.ontouchend = e => { e.preventDefault(); launch(); };
  if (RESET) RESET.onclick = () => release('manual');

  window.__ULTIMATE_INSANITY_V2 = { launch, release, isRunning: () => running };

  // Debug/test hook: opening index.html#test-insane immediately runs the mode.
  // It is intentionally hash-only so normal visitors never trigger it accidentally.
  if (location.hash === '#test-insane') {
